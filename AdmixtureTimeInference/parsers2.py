...
def phys2gen_file(bpfile,mapfile):
    ...
    Returns:
    NULL. Writes a new bpfile (bpfile_out.bp) with genetic instead of physical positions
    '''
...
#{{{
    ...
    with open(bpfile) as f:
        for count, bpline in enumerate(f):
            ...
            for i in range(len(parsed_list)):
                ...
                for p in physical_positions:
                    ...
                    if not p == int(mapDF_chrom.pos[indx]): #if position is not found exactly 
                        num = float(mapDF_chrom.sex_avg[indx]) - float(mapDF_chrom.sex_avg[indx-1])
                        denom = int(mapDF_chrom.pos[indx]) - int(mapDF_chrom.pos[indx-1])
                        slope = num/denom
                        geneticpos_interp = round(float(mapDF_chrom.sex_avg[indx-1]) + slope*(p - int(mapDF_chrom.pos[indx-1])),3)
                     
                    else:
                        geneticpos_interp = round(float(mapDF_chrom.sex_avg[indx]),3)  
            ...
            #        print(mapDF_chrom.iloc[indx], mapDF_chrom.iloc[indx])
...
if __name__ == "__main__":
    ...
    print("Running phys2gen_file with arguments = {}, {}".format(args.inputfile,args.mapfile))    
    phys2gen_file(args.inputfile, args.mapfile)
