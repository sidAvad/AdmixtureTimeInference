#$ -S /bin/bash      # Use bash
#$ -N simulate_homozyg-tracts  # name for job in qstat/output filename prefix
#$ -o /fs/cbsubscb09/storage/siddharth/AdmixtureTimeInference/simulate_homozyg-tracts_2gens.$JOB_ID
...
RES_DIR=~/siddharth/AdmixtureTimeInference/Results/homozyg-tract-lengths
...
do
    ...
    python AdmixtureTimeInference/get_homozyg_tracts.py -i "$BP_FILE"-focal.gen -a "$typ" -l Resources/${t}_admixturelabels.txt -o ${RES_DIR}/${t}gens/admixture-time_"$t"gens_type${typ}_${pedsim_postfix}.res
done    
